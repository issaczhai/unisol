...
<div class="panel panel-default referral-coupon">
  ...
  <div class="panel-body">
    ...
    <div class="referral-coupon-form table-responsive">
      ...
      <div class="text-info"><?php echo $sending_limit['text']; ?></div>
      <form>
        <div class="form-group" style="display:none">
          <?php echo $lng['entry_message']; ?><br/>
          <textarea name="referrer_message" class="referrer_message form-control">Congratulations!</textarea>
        </div>
  ...
  $.ajax({
    ...
    error: function(xhr, ajaxOptions, thrownError) {
		...
		success: function(json) {
	      ...
	      if (referee.error.limit_reached) {
	        $(obj).parents('.referral-coupon-form').prepend('<div class="alert alert-danger"><i class="fa fa-exclamation-circle"></i> ' + referee.error.limit_reached + '</div>');
	        return false;
	      } else {
	        $(obj).parents('table').find('tr.referee').each(function() {
